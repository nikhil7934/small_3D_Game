...
Ball::Ball(float x, float y, float z,float side,color_t color) {
...
void Ball::tick() {
    ...
    if(this->back)
    {
        this->position.z += 0.2*(cos(this->dirroc*(M_PI/180.0f)));
        this->position.x += 0.2*(sin(this->dirroc*(M_PI/180.0f)));
    }
    else
    {
        this->position.z -= 0.2*(cos(this->dirroc*(M_PI/180.0f)));
